...
public class ElementUtil {
    ...
    public void checkContainsGetTitle(String text) {Assert.assertTrue(driver.getTitle().contains(text));}
    ...
    public void clickElementWithTextContainsFromElements(By key, String text) {
        boolean check = false;
    ...
    public void checkElementFromParentForCheckCorrectPage() {checkElementVisible(returnElement.findElement(By.xpath("..")).findElement(By.cssSelector(".dropdown-menu.show")));}

    public void checkSelectedWithTextContainsFromElements(By key, String text) {
        boolean check = false;
        for(WebElement element : presenceElements(key)){
            if(element.getText().contains(text)){
                element.isSelected();
    ...
    public void checkAllElementContainsTextFromElements(By key, String text) {
        for(WebElement element : presenceElements(key)){
            if(!element.getText().contains(text)){
                Assert.fail(element.getText() + " does not contain the text \"" + text + "\"");
            }
        }
    }
    ...
    public void sendKey(By key, String string) {
        try {
            presenceElement(key).sendKeys(string);
        } catch (Exception ignored) {
    ...
    public void checkElementText(By by,String text) {
        Assert.assertEquals(text, elementGetText(by));
    }
